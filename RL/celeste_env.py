...
class CelesteGymEnv(gym.Env):
    ...
    def calculate_reward(self, current_player_y):
        ...
        # Different penalties and rewards based on movement type
        if current_player_y > self.previous_player_y:
            # Penalty for moving down - make it more significant
            height_reward -= 3
        elif current_player_y == self.previous_player_y:
            # Small penalty for not moving vertically (staying in place)
            height_reward -= 0.05

        # Large penalty for dying (falling off the screen)
        if current_player_y > 16 or player is None:  # Player fell off the bottom or dead
            height_reward -= 200  # Increased penalty to strongly discourage dying

        # Bonus for reaching higher positions
        if current_player_y < 0:  # Reached the top goal
            height_reward += 200  # Large reward for completing the level
        elif current_player_y < 1:
            height_reward += 50
        elif current_player_y < 5:
            height_reward += 20
        elif current_player_y < 10:
            height_reward += 5

        # Progressive reward based on how high the player has reached
        progress_reward = max(0, (self.initial_player_y - current_player_y) * 2)  # Encourage progress
        height_reward += progress_reward

        # Small positive reward for staying alive (encourage exploration)
        height_reward += 0.05

        # Return normalized reward
        return height_reward / 10.0
    ...
    def is_truncated(self):
        ...
        current_player_y = self.get_player_position()[1]
        # Truncate if player falls too far down OR if max steps reached
        return current_player_y > 16 or self.current_step >= self.max_steps
